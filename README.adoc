...
- `functional` is when working with functional bean registration (https://github.com/spring-projects-experimental/spring-fu[Spring Fu] style or using https://github.com/spring-projects-experimental/spring-init/[spring-init]). In this mode the feature will provide initialization, resource configuration, substitutions and `spring.factories` functional alternative (only Spring Boot `spring.factories` are supported for now).
