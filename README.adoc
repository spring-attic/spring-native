...
- Run `docker/build-{graalvm-dev-version}.sh` to build the Docker images
- You can now invoke `run-{graalvm-dev-version}-java8.sh` or `run-{graalvm-dev-version}-java11.sh` to run the Docker container suitable to run `spring-graal-native` build scripts.
