...
It supports the {documentation-url}/reference/index.html#_feature_options[following modes]:

- `feature` is the default mode, it provides resource, initialization, proxy and reflection (using auto-configuration hints) configuration for native images as well as substitutions.
...
- `functional` is when working with functional bean registration (https://github.com/spring-projects-experimental/spring-fu[Spring Fu] style). In this mode the feature will provide initialization, resource configuration and substitutions.
