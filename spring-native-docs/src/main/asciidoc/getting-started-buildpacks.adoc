...
NOTE: You can run `docker run hello-world` (without `sudo`) to check the Docker daemon is reachable as expected, see related https://docs.spring.io/spring-boot/docs/{spring-boot-version}/maven-plugin/reference/htmlsingle/#build-image-docker-daemon[Maven] or https://docs.spring.io/spring-boot/docs/{spring-boot-version}/gradle-plugin/reference/htmlsingle/#build-image-docker-daemon[Gradle] Spring Boot plugin documentation.
