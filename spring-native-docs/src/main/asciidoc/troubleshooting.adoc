...
You have probably tried to compile a Spring Boot native application without the `spring-native` dependency.
...
* Raise a bug against the `spring-native` project, as a key aim of the feature is to make sure these things get registered.
If it is not "`obvious`" that it should be registered, it may be necessary for a new or expanded hint to the added to the `spring-native-configuration` project (see the <<how-to-contribute,how to contribute guide>> if you want to explore that).
...
The scripts folder in `spring-native` contains a compare script, which you can invoke as follows:

====
[source,bash]
----
~/spring-native/scripts/reflectCompare.sh feature-reflect-config.json native-image-config/reflect-config.json > diff.txt
----
====
