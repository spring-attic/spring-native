...
- new in 0.9.0 - using <<getting-started-build-time-infrastructure-native-image, GraalVM native image Maven plugin support>> to generate a native executable. This mode is using spring-native as a build time configuration generator and *not* as a GraalVM native image feature plugin.
