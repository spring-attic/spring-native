...
Be aware of https://youtrack.jetbrains.com/issue/KT-44594[that Kotlin bug] which requires additional array reflection entries, it should be fixed in Kotlin 1.5+. Kotlin Coroutines are supported but require additional reflection entries due to how Coroutines generates bytecode with an `Object` return type.
...
** JMX support is disabled because it is incompatible with the build time initialization currently required for logging.
** The footprint is not optimal yet due to the inclusion `GraalJSEngineFactory`, see https://github.com/oracle/graal/issues/2991[graal#115] for more details.
...
* `spring-boot-starter-quartz`: supports the [Quartz Job Scheduling](http://www.quartz-scheduler.org/) engine. It adds types required by Quartz, and automatically registers any `Job` subclasses for reflection.

IMPORTANT: Devtools is not supported yet, you can follow
https://github.com/spring-projects-experimental/spring-native/issues/532[#532] to be aware when it will.
...
CGLIB proxies on classes are not supported, only JDK dynamic proxies on interfaces are for now. As a consequence `spring.aop.proxy-target-class` is set to `false` when using Spring Native. You can subscribe to and vote for https://github.com/spring-projects-experimental/spring-native/issues/356[#356] if you are interested about proxy support on classes.
