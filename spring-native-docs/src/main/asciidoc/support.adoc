...
CGLIB proxies on classes are not supported, only JDK dynamic proxies on interfaces are for now. As a consequence `spring.aop.proxy-target-class` is set to `false` when using Spring Native. You can subscribe and vote for https://github.com/spring-projects-experimental/spring-native/issues/356[#356] if you are interested about proxy support on classes.
