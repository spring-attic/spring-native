...
----
    <plugin>
		...
		<configuration>
			<argLine>${build.args} -agentlib:native-image-agent=access-filter-file=src/test/resources/access-filter.json,config-merge-dir=target/classes/META-INF/native-image</argLine>
		</configuration>
	</plugin>
----
====

Also update the `native-image` arguments to enable the agent mode with `-Dspring.native.mode=agent`.
