...
For a milestone of Spring Boot it is likely necessary to add these repositories to the pom:
====
[source,xml,subs="attributes,verbatim"]
----
<pluginRepositories>
      ...
      <plugins>
        <plugin>
          ...
          <configuration>
            ...
            <buildArgs>-Dspring.native.mode=init</buildArgs>
          </configuration>
          <executions>
            <execution>
              <goals>
...
IMPORTANT-4: For more info on trying to use tests with the agent to generate the necessary configuration,
see <<testing_and_the_agent.adoc>>.
