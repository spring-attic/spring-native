...
So basically, to compile to native applications, your applications need to switch to using `proxyBeanMethods=false` on `@SpringBootApplication` and `@Configuration`.
