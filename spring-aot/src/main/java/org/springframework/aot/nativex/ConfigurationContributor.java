...
/**
 * Contributes the configuration files for native image construction. This
 * includes the reflection, resource and proxy configuration in addition to the
 * native-image.properties file that includes special args and initialization
 * configuration.
 * 
 * @author Andy Clement
 */
public class ConfigurationContributor implements BootstrapContributor {
	...
	public void contribute(BuildContext context, AotOptions aotOptions) {
		...
		context.describeJNIReflection(
				jniReflect -> jniReflect.merge(configurationCollector.getJNIReflectionDescriptor()));
			...
			context.addResources(new ResourceFile() {
				...
				public void writeTo(Path resourcesPath) throws IOException {
					...
					try (FileOutputStream fos = new FileOutputStream(
							resourcesPath.resolve(ResourceFile.SPRING_COMPONENTS_PATH).toFile())) {
	...
	/**
	 * If the configuration has requested any class proxies to be constructed at
	 * build time, create them and register reflective access to their necessary
	 * parts.
	 */
	private void processBuildTimeClassProxyRequests(BuildContext context,
		...
		if (!classProxyNames.isEmpty()) {
			for (String classProxyName : classProxyNames) {
				...
				// TODO [build time proxies] not all proxy variants will need method access -
				// depends on what
				// it is for - perhaps surface access bit configuration in the classproxyhint to
				// allow flexibility?
	...
	public List<String> generateBuildTimeClassProxies(ConfigurationCollector configurationCollector,
