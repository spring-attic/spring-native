...
class PrivateFactoriesCodeContributorTests {
	...
	@Test
	void shouldNotContributeWhenPublicConstructor() {
		SpringFactory factory = SpringFactory.resolve(TestFactory.class.getName(), PublicFactory.class.getName(), PrivateFactoriesCodeContributorTests.class.getClassLoader());
		assertThat(this.contributor.canContribute(factory)).isFalse();
	}

	@Test
	void shouldContributeWhenClassIsProtected() throws Exception {
		SpringFactory factory = SpringFactory.resolve(TestFactory.class.getName(),
				"org.springframework.aot.factories.fixtures.ProtectedFactory", PrivateFactoriesCodeContributorTests.class.getClassLoader());
		assertThat(this.contributor.canContribute(factory)).isTrue();
	}

	@Test
	void shouldContributeStaticStatement() throws Exception {
		...
		factory = SpringFactory.resolve(TestFactory.class.getName(),
				"org.springframework.aot.factories.fixtures.ProtectedFactory.InnerProtectedFactory", PrivateFactoriesCodeContributorTests.class.getClassLoader());
		this.contributor.contribute(factory, code, Mockito.mock(BuildContext.class));
