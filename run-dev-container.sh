...
while test $# -gt 0; do
  case "$1" in
    -h|--help)
      ...
      echo "-g, --graalvm=VERSION     specify GraalVM version to use, can be 20.2-dev or master, 20.2-dev by default"
...
docker run --hostname docker -p 8080:8080 -v $HOST_WORK_DIR:$CONTAINER_WORK_DIR:delegated -v $HOME/.m2:$CONTAINER_HOME/.m2:delegated -it -w $CONTAINER_WORK_DIR -u $(id -u ${USER}):$(id -g ${USER}) --group-add elasticsearch spring-graalvm-native-dev:${GRAALVM_VERSION}-java${JAVA_VERSION}